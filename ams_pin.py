...
class VirtualPin:
    ...
    def __init__(self, printer, name):
        ...
        try:
            ...
            # FakePrinter in tests may not implement register_event_handler
            pass
...
if _pins_module is not None and not getattr(_pins_module.Pins.parse_pin, "_ams_pin_patch", False):
    _orig_parse_pin = _pins_module.Pins.parse_pin

    def _patched_parse_pin(self, pin_desc, can_invert=False, can_pullup=False):
        desc = str(pin_desc).lstrip("!^")
        if desc.startswith("ams_pin:") and "ams_pin" not in self.chips:
            try:
                get_virtual_pins(self.printer)
            except Exception:
                pass
        return _orig_parse_pin(self, pin_desc, can_invert, can_pullup)

    _patched_parse_pin._ams_pin_patch = True
    _pins_module.Pins.parse_pin = _patched_parse_pin
